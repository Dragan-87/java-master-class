...
2. Create a package with your name and move `com.dragan.Main.java` inside it:

```
src/com/<yourname>/com.dragan.Main.java
```

For example, if your name is Franco: `src/com/franco/Main.java`

The starter `com.dragan.Main.java` contains TODOs to guide the first steps:
...
graph TB
    subgraph Presentation
        M[com.dragan.Main.java<br>CLI Menu Loop]
    end
...
- Implement the CLI menu loop in `com.dragan.Main.java`
...
**Wiring in com.dragan.Main.java** — the student can now swap between array and file implementations for bookings:
...
These can be swapped in via dependency injection in `com.dragan.Main.java`:
...
│   │   └── com/
│   │       └── <yourname>/
│   │           ├── com.dragan.Main.java                       (@SpringBootApplication)
│   │           ├── car/
│   │           │   ├── Brand.java
│   │           │   ├── Car.java                    (@Entity)
